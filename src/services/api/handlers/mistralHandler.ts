...
export async function handleMistralApi(messages: Message[], config: ApiConfig): Promise<ModelResponse> {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error?.message || `Mistral API request failed: ${response.statusText}`);
    }
